...
async def ship_product(ctx: SlashContext, item_name: str, quantity: int):
    ...
    if item_name in shipped_inventory:
        shipped_inventory[item_name]['quantity'] += quantity
        shipped_inventory[item_name]['serial_numbers'].extend(serial_numbers_to_ship)
    else:
        shipped_inventory[item_name] = {'quantity': quantity, 'serial_numbers': serial_numbers_to_ship}
...
@slash.slash(name="shipped_inventory", description="查看所有被出貨過的庫存")
@commands.check(is_admin)
async def shipped_inventory_command(ctx: SlashContext):
    if shipped_inventory:
        shipped_list = "\n".join([f"{item}: 數量: {details['quantity']}, 序號: {', '.join(details['serial_numbers'])}" for item, details in shipped_inventory.items()])
        await ctx.send(f"已出貨庫存:\n{shipped_list}")
    else:
